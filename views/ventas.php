...
<div class="container my-4">
  ...
  <div class="d-flex flex-column flex-md-row justify-content-between align-items-stretch mb-3">
    <h1 class="mb-3 mb-md-0 fs-3">Gestión de ventas</h1>
    <div class="d-flex flex-column flex-sm-row gap-2">
      ...
      <div class="col-md-auto">
        <a href="agregar_venta.php" class="btn btn-primary w-100 rounded-pill">Agregar nueva venta</a>
      </div>
    </div>
  </div>
      ...
      <tbody>
        ...
        if ($resultado->num_rows > 0) {
          while ($fila = $resultado->fetch_assoc()) {
            ...
            echo "<td>";
            if ($rol === 'Admin') {
              echo "<a href='editar_venta.php?id={$fila['ID_orden_venta']}' class='btn btn-sm btn-warning'>Editar</a> 
                    <a href='../controllers/eliminar_venta.php?id={$fila['ID_orden_venta']}' onclick=\"return confirm('¿Estás seguro?')\" class='btn btn-sm btn-danger'>Eliminar</a>";
            } else {
              echo "Sin permisos";
            }
            echo "</td>";
