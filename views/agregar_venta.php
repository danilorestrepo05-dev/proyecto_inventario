...
<script>
    ...
    document.getElementById('formOrden').addEventListener('submit', function(e) {
        ...
        localStorage.removeItem('svt_venta_draft');
    });

    // === LOCALSTORAGE: Guardar/Cargar borrador ===
    const STORAGE_KEY = 'svt_venta_draft';
    ...
    // Al cargar la página: restaurar borrador si existe
    document.addEventListener('DOMContentLoaded', function() {
        const raw = localStorage.getItem(STORAGE_KEY);
        if (raw) {
            if (confirm('Tienes una venta sin guardar. ¿Deseas restaurarla?')) {
                cargarBorrador();
            } else {
