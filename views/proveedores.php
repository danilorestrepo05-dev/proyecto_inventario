...
<div class="container my-4">
  ...
  <div class="d-flex flex-column flex-md-row justify-content-between align-items-stretch mb-3">
    <h1 class="mb-3 mb-md-0 fs-3">Gestión de proveedores</h1>
    <div class="d-flex flex-column flex-sm-row gap-2">
      ...
      <div class="col-md-auto">
        <a href="agregar_proveedor.php" class="btn btn-primary w-100 rounded-pill">Agregar proveedor</a>
      </div>
    </div>
  </div>
      ...
      <tbody>
        ...
        if ($resultado->num_rows > 0) {
          while ($fila = $resultado->fetch_assoc()) {
            ...
            echo "<td>";
            if ($rol === 'Admin') {
              echo "<a href='editar_proveedor.php?id={$fila['ID_proveedor']}' class='btn btn-sm btn-warning'>Editar</a> 
                    <a href='../controllers/eliminar_proveedor.php?id={$fila['ID_proveedor']}' onclick=\"return confirm('¿Estás seguro de eliminar este proveedor?')\" class='btn btn-sm btn-danger'>Eliminar</a>";
            } else {
              echo "Sin permisos";
            }
            echo "</td>";
